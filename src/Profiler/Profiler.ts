...
export function init(): Profiler {
  ...
  function record(key: string | symbol, time: number) {
    if (!Memory.profiler.data[key as string]) {
      Memory.profiler.data[key as string] = {
        calls: 0,
        time: 0,
      };
    }
    Memory.profiler.data[key as string].calls++;
    Memory.profiler.data[key as string].time += time;
  }
